...
describe("admin layout stress guardrails", () => {
  ...
  it("collapses shell and dashboard structure at tablet widths", () => {
    ...
    expect(css).toMatch(
      /@media \(max-width: 1080px\)[\s\S]*\.hero-panel\s*{[^}]*grid-template-columns:\s*1fr;/s,
    );
  ...
  it("lays dashboard out with snapshot first and full-width charts", () => {
    ...
    expect(
      dashboardPage.indexOf('className="dashboard-alerts-panel"'),
    ).toBeLessThan(dashboardPage.indexOf('className="stat-grid"'));
    ...
    expect(css).toMatch(
      /\.dashboard-alerts-panel\s*{[^}]*grid-column:\s*span 8;/s,
    );
    expect(css).toMatch(/\.stat-grid\s*{[^}]*grid-column:\s*span 4;/s);
    expect(css).toMatch(
      /\.stat-grid\s*{[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);/s,
    );
    expect(css).toMatch(
      /\.dashboard-alerts-panel \.dashboard-alert-list\s*{[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);/s,
    );
    expect(css).toMatch(/\.hero-panel\s*{[^}]*grid-column:\s*1 \/ -1;/s);
    expect(css).toMatch(/\.hero-panel\s*{[^}]*min-height:\s*178px;/s);
    expect(css).toMatch(
  ...
  it("styles native selects as dense admin dropdowns", () => {
    ...
    expect(css).toMatch(
      /select:focus\s*{[^}]*--select-arrow:\s*var\(--primary\);/s,
    );
